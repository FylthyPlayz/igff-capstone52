...
const useSimpleAuth = () => {
    ...
    const isAuthenticated = () => localStorage.getItem("IGFF_token") !== null
        || sessionStorage.getItem("IGFF_token") !== null
    ...
    const register = (user) => {
        ...
        .then(response => {
            if ("id" in response) {
                ...
                localStorage.setItem("IGFF_token", encoded)
            }
        })
    }
    ...
    const logout = () => {
        ...
        localStorage.removeItem("IGFF_token")
        sessionStorage.removeItem("IGFF_token")
    }

    const getCurrentUser = () => {
        const encoded = localStorage.getItem("IGFF_token")
